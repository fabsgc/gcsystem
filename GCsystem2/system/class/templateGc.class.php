<?php
	...
	class templateGcParser{
		...
		private function parsevars(){
			...
			$this->contenu = preg_replace('`'.preg_quote($this->bal['vars'][0]).'(.+)'.preg_quote($this->bal['vars'][1]).'`', '<?php echo $$1; ?>', $this->contenu);
		}
		
		private function parsevarsExist(){
			$this->contenu = preg_replace('`'.preg_quote($this->bal['vars'][2]).'(.*)'.preg_quote($this->bal['vars'][3]).'`', '<?php echo $1; ?>', $this->contenu);
		}
