<?php
	...
	*/
	...
	class terminalGc{
		use errorGc, langInstance, generalGc, helperLoader;                  //trait
		...
		public  function __construct($command, $bdd, $lang = 'fr'){
			...
			$this->_forbidden = array(
                ...
                CLASS_CRON, CLASS_INSTALL, CLASS_ANTISPAM, CLASS_FIREWALL, CLASS_APPLICATION, CLASS_ROUTER, CLASS_AUTOLOAD, CLASS_GENERAL_INTERFACE,CLASS_CONTROLLER, CLASS_LOG, CLASS_CACHE, CLASS_EXCEPTION, CLASS_TEMPLATE,CLASS_LANG, CLASS_APPDEVGC, CLASS_TERMINAL, CLASS_ERROR_PERSO,
				...
				ROUTE, MODOGCCONFIG, APPCONFIG, HELPER, FIREWALL, ASPAM, INSTALLED, CRON, ERRORPERSO
			);
					...
					if(!in_array(CONTROLLER_PATH.$this->_commandExplode[2].CONTROLLER_EXT.'.php', $this->_forbidden) && !in_array(MODEL_PATH.$this->_commandExplode[2].MODEL_EXT.'.php', $this->_forbidden)){
						if(!file_exists(MODEL_PATH.$this->_commandExplode[2].MODEL_EXT.'.php')){
							$monfichier = fopen(CONTROLLER_PATH.$this->_commandExplode[2].CONTROLLER_EXT.'.php', 'a');						
								$t= new templateGC(GCSYSTEM_PATH.'GCcontroller', 'GCcontroller', '0');
								$t->assign(array(
