		...
		class editor{
			...
			*/
			...
			public function parse($contenu = ""){
				...
				$this->_contenu =  preg_replace_callback('#^((?:https?|ftp)://\S+?)(?=[]\).,;:!?]?(?:\s|\Z)|\Z)#isU',
					array('helper\editor', '_linkBegin'), $this->_contenu);

				$this->_contenu =  preg_replace_callback('#\n((?:https?|ftp)://\S+?)(?=[]\).,;:!?]?(?:\s|\Z)|\Z)#isU',
					array('helper\editor', '_linkLine'), $this->_contenu);

				$this->_contenu =  preg_replace_callback('# ((?:https?|ftp)://\S+?)(?=[]\).,;:!?]?(?:\s|\Z)|\Z)#isU',
					array('helper\editor', '_linkText'), $this->_contenu);

				/* ############################### CODE ###################### */
				$this->_contenu = preg_replace_callback(
					'`'.preg_quote(self::TAGSTART).'code type=&quot;(.*)&quot;'.preg_quote(self::TAGEND).'(.*)'.preg_quote(self::TAGSTART2).'code'.preg_quote(self::TAGEND).'`isU', 
					array('helper\editor', '_codeInArray'), $this->_contenu
				);
				...
				$this->_contenu = preg_replace_callback(
					...
					array('helper\editor', '_video'), $this->_contenu
				);
				...
				$this->_contenu = preg_replace_callback(
					'`\[code id=(.+)\]`isU', 
					array('helper\editor', '_codeInString'), $this->_contenu
				);

				$this->_contenu = preg_replace_callback(
					'`'.preg_quote(self::TAGSTART).'code type=&quot;(.*)&quot;'.preg_quote(self::TAGEND).'(.*)'.preg_quote(self::TAGSTART2).'code'.preg_quote(self::TAGEND).'`isU', 
					array('helper\editor', '_code'), $this->_contenu
				);
			...
			*/
			...
			public function editor($contenu, $option = array()){
				...
				$tpl = new \system\template(USER_PREFIXE_TEMPLATE.'/include/editor/default', 'editor_'.$this->_id, 0, $this->_lang);
				$tpl->assign(array(
					'message' => $contenu,
					'id' => $this->_id,
					'name' => $this->_name,
