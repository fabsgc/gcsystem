<?php
	...
	namespace helper{
		class bbcode{
			use \system\error, \system\langInstance;
			...
			*/
			...
			/**
			 * créé une instance de la classe lang
			 * @access public
			 * @return void
			 * @since 2.0
			*/
			...
			protected function _createLangInstance(){
				$this->_langInstance = new \system\lang($this->_lang);
			}
			...
			*/
			...
			public function parse($contenu = ""){
				...
				$this->_contenu = preg_replace_callback(
					'`\[code id=(.+)\]`isU', 
					array('helper\bbcode', '_codeInString'), $this->_contenu
				);

				$this->_contenu = preg_replace_callback(
					'`'.preg_quote(self::TAGSTART).'code type=&quot;(.*)&quot;'.preg_quote(self::TAGEND).'(.*)'.preg_quote(self::TAGSTART2).'code'.preg_quote(self::TAGEND).'`isU', 
					array('helper\bbcode', '_code'), $this->_contenu
				);
			...
			*/
			...
			public function editor($contenu, $option = array()){
				...
				$tpl->assign(array(
					...
					'instantaneous' => $this->_previewInstantanee,
