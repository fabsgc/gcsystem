	...
	class Engine{
		...
		public function _action(&$class){
			ob_start();
				...
				if(method_exists($class, 'action'.ucfirst($this->request->action))){
					...
					throw new Exception('The requested action "'.$this->request->action.'" from "'.$this->request->controller.'" doesn\'t exist');
				}
		...
		*/
		...
		private function _setCron($src = null){
			if($src == null)
				new Cron($this->profiler, $this->config, $this->request, $this->lang, APP_CONFIG_CRON);
			else
				new Cron($this->profiler, $this->config, $this->request, $this->lang, SRC_PATH.$src.'/'.SRC_CONFIG_CRON);
		}
